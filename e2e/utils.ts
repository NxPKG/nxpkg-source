...
export function newApp(name: string): string {
  ...
  // return execSync(`../../node_modules/.bin/schematics @nxpkg/schematics:app --name=${name}
  // --collection=@nxpkg/schematics`, { cwd: `./tmp/${projectName}` }).toString();
}

// switch to ng generate, once CLI is fixed
export function newLib(name: string): string {
  return runCLI(`generate lib ${name}`);
  // return execSync(`../../node_modules/.bin/schematics @nxpkg/schematics:lib --name=${name}
  // --collection=@nxpkg/schematics`, { cwd: `./tmp/${projectName}` }).toString();
}
...
export function copyMissingPackages(): void {
  const modulesToCopy = ['@ngrx', 'jasmine-marbles', '@nxpkg', 'angular', '@angular/upgrade', '@angular/cli'];
