...
describe('NxPKG Convert to NxPKG Workspace', () => {
  beforeEach(cleanup);
  ...
  it('should generate a workspace', () => {
    ...
    // confirm that @nxpkg and @ngrx dependencies do not exist yet
    expect(packageJson.devDependencies['@nxpkg/schematics']).not.toBeDefined();
    expect(packageJson.dependencies['@nxpkg/nxpkg']).not.toBeDefined();
    ...
    // run the command
    runCLI('generate workspace proj --npmScope=proj --collection=@nxpkg/schematics');
    ...
    expect(updatedPackageJson.devDependencies['@nxpkg/schematics']).toBeDefined();
    expect(updatedPackageJson.dependencies['@nxpkg/nxpkg']).toBeDefined();
    ...
    expect(updatedAngularCLIJson.defaults.schematics.collection).toEqual('@nxpkg/schematics');
  ...
  it('should generate a workspace and not change dependencies or devDependencies if they already exist', () => {
    ...
    ngNew('--skip-install');
    const nxpkgVersion = '0.0.0';
    const schematicsVersion = '0.0.0';
    const ngrxVersion = '0.0.0';
    // update package.json
    const existingPackageJson = JSON.parse(readFile('package.json'));
    existingPackageJson.devDependencies['@nxpkg/schematics'] = schematicsVersion;
    existingPackageJson.dependencies['@nxpkg/nxpkg'] = nxpkgVersion;
    ...
    // run the command
    runCLI('generate workspace proj --collection=@nxpkg/schematics');
    // check that dependencies and devDependencies remained the same
    const packageJson = JSON.parse(readFile('package.json'));
    expect(packageJson.devDependencies['@nxpkg/schematics']).toEqual(schematicsVersion);
    expect(packageJson.dependencies['@nxpkg/nxpkg']).toEqual(nxpkgVersion);
